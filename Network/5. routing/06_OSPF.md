...
OSPF는 RIP처럼 Hop Count가 아니라 Cost를 사용하며 Cost는 대역폭을 기준으로 계산된다.

Cost 값이 낮을수록 더 좋은 경로이다.
...
```text
192.168.53.0/24 → Area 0 등록

1.1.1.0/24 → Area 0 등록
```
...
110
→ Administrative Distance(OSPF 기본값)
...
실무에서는 인터넷이나 다른 라우팅 프로토콜과 연결할 때 사용한다.
...
- ABR은 Area와 Area를 연결하는 Router이다.
